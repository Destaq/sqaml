...
(**String utility function to implement a classical replace all.*)
...
(**Split query string into a list of tokens that can be interpreted elsewhere.*)
...
(**Verify that the order of columns passed in an insert query is correct for a given table.*)
...
(**Parse a full create table query, as well as an insert query.*)
...
(**Check whether a statement includes a where clause.*)
...
(**Get a list of fields to update from an update query.*)
...
(**Return an operation function associated with a specific operation operation string.*)
let get_op_value op =
  match op with
  | "=" -> Row.value_equals
...
(**Construct parameters from a token list to construct a predicate or where clause.*)
...
(**Construct parameters for building out a data transformation or a table update.*)
...
(**Parse the query fields in a select query to facilitate dynamic field selection.*)
...
(**Extract column names from a list of fields.*)
...
(**Check whether a limit clause exists and return the limit number if so.*)
...
(**Check whether an order by clause exists and return the corresponding data if so.*)
...
(**Construct a sorting function for order by.*)
...
(**Primary query parser and main gateway.*)
...
(**Main gateway to the backend query parsing logic.*)
